...
function calculatePercentage(indexValue: number) {
  return Math.round((((indexValue+1) / data.length) * 100));
}

export const groupedData = () => data.reduce((acc, value, idx) => {
  ...
  return {
    ...
    [value.slug]: {
      ...value,
      order: idx,
      percentage: `${calculatePercentage(idx)}%`,
